...
-- 거래처 정보 테이블 (상품을 납품하는 거래처)
CREATE TABLE buyer
(  buyer_id           CHAR(6)       NOT NULL,   --거래처코드 
   buyer_name         VARCHAR2(50)  NOT NULL,   --거래처명
   buyer_lgu          CHAR(4)       NOT NULL,   --상품분류코드
   buyer_bank         VARCHAR2(40),            --은행명
 ...
 ALTER TABLE buyer add ( buyer_mail VARCHAR2(40), -- 메일주소
                         buyer_charger VARCHAR2(10), -- 거래처 담당자
                         buyer_telext VARCHAR2(2)); -- 회사전화번호
 ...
 ALTER TABLE buyer
   add ( CONSTRAINT pk_buyer PRIMARY KEY(buyer_id),
           CONSTRAINT fr_buyer_lgu  foreign key(buyer_lgu) 
                               references lprod(lprod_gu) ); -- lprod라는 테이블의 PK(col명 : lprod_gu)를 참조해서 FK를 만들겠다

alter table buyer 
	add constraint Check_Phone 
					check(buyer_telext like '[0-9][0-9]');

-- 목차를 만들어서 buyer_name, buyer_id로 무언가 찾고자 할 때 빠르게 확인하기 위해 만드는 것
...
(  prod_id             VARCHAR2(10)     NOT NULL,     -- 상품코드
   ...
   prod_lgu            CHAR(4 )         NOT NULL,     -- 상품분류코드
   prod_buyer          CHAR(6)          NOT NULL,     -- 거래처코드
   prod_cost           NUMBER(10)       NOT NULL,     -- 매입가격
   prod_price          NUMBER(10)       NOT NULL,     -- 소비자가격
   prod_sale           NUMBER(10)       NOT NULL,     -- 판매가격
   prod_outline        VARCHAR2(100)     NOT NULL,    -- 상품개략설명
   prod_detail         CLOB,                          -- 상품상세설명
   prod_img            VARCHAR2(40)     NOT NULL,     -- 이미지 파일명(소)
